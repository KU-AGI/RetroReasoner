...
class localmapper:
    def __init__(self, device='cpu', model_version='202403'):
        ...
        self.LRT_extractor = lambda rxn: extract_from_reaction(rxn, {'include_neighbors': False})
        self.ELRT_extractor = lambda rxn: extract_from_reaction(rxn)
    ...
    def get_atom_map(self, rxns, return_dict=False, show_LRT=False):
        ...
        for i, (rxn, prediction) in enumerate(zip(rxns, predictions)):
            mapped_result = prediction2map(rxn, prediction, self.ELRT_extractor)
            result = {'rxn': rxn}
            result.update(mapped_result)
            confident = result['template']['canonical'] in self.accepted_templates
            if not confident:
                mapped_result = prediction2map(rxn, prediction, self.ELRT_extractor, 90)
                result.update(mapped_result)
                confident = result['template']['canonical'] in self.accepted_templates
            ...
            if not return_dict:
                ...
                origin_template, canonical_template, atoms_to_use_reactant, atoms_to_use_product = self.LRT_extractor(mapped_result['mapped_rxn'])
                template = {
                    "origin": origin_template,
                    "canonical": canonical_template,
                    "atoms_to_use_reactant": atoms_to_use_reactant,
                    "atoms_to_use_product": atoms_to_use_product
                }
